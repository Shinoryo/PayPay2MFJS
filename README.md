...
| 引数 | 必須 | 説明 |
| ---- | ---- | ---- |
| `--csv=<path>` | 必須 | PayPay 利用明細 CSV ファイルパス |
...
11. 日付入力後は datepicker が残っている場合に備えて、保存前にクローズを試みる。
    blur/Tab/モーダル内クリック/非表示待機の各ステップに closeWaitMs を適用し、
    blur ステップの `Locator.evaluate` は第3引数の options で timeout を指定する。
12. 登録成功後に重複履歴を更新する。
...
| バージョン | 日付 | 内容 |
| ----- | ---------- | -------------- |
| 1.1.0 | 2026-05-10 | 振替も自動登録できる機能を追加<br>Node.js 24 対応 |
